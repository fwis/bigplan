class World {
  constructor(worldSize, miniMapCanvas, trendChart) {
      ...
      // 初始化四叉树
      this.quadTree = new QuadTree(new Rectangle(0, 0, worldSize / 2, worldSize / 2), 4);
  }
  ...
  CreateWorm(x, z, rotation, usingInstance) {
      ...
      // 插入到四叉树
      this.quadTree.insert({ x: wormMesh.position.x, y: wormMesh.position.z, worm });
  ...
  _buildDenseGrassMesh() {
      const items = [];
      for (var i = 0; i < 7; i++) {
          var grass = this._buildGrassMesh0();
  ...
  CreateSingleGrass(x, z, rotation) {
      ...
      // 插入到四叉树
      this.quadTree.insert({ x: grassMesh.position.x, y: grassMesh.position.z, grass });
  ...
  CreateDenseGrass(x, z, rotation) {
      ...
      // 插入到四叉树
      this.quadTree.insert({ x: denseGrassMesh.position.x, y: denseGrassMesh.position.z, grass });
  ...
  CreateFlowerGrass(x, z, rotation) {
      ...
      // 插入到四叉树
      this.quadTree.insert({ x: flowerGrassMesh.position.x, y: flowerGrassMesh.position.z, grass });
  ...
  _isInExistingArea(x, z) {
      ...
      const range = new Rectangle(x, z, minDistance, minDistance);
